...
import { ISaveSitemanagerUseCase } from "../../interfaces/AdminUseCaseEntities/SiteUseCaseEntities/SaveSitemanagerEntity"
...
export class SaveSitemanagerUseCase implements ISaveSitemanagerUseCase {
   ...
   async execute(input: addsitemanagerInput): Promise<commonOutput> {
      ...
      const existSitemanager = await this._sitemanagerRepository.findSitemanagerByEmail(email)
      if (existSitemanager) {
         return ResponseHelper.conflictData(SitemanagerFailedMessage.EXIST)
      }
      const password = await this._sitemanagerRepository.generatePassword()
      ...
      await this._sitemanagerRepository.saveSitemanager({ username, email, password: hashpassword })
