...
import { IFetchStatusBaseProjectUseCase } from "../../interfaces/UserUseCaseEntities/ProjectDisplayUseCaseEntities/FetchStatusBaseProjectUseCaseEntity";
...
export class FetchStatusBaseProjectUseCase implements IFetchStatusBaseProjectUseCase {
   ...
   async execute(input: fetchprojectInput): Promise<commonOutput<{ projectData: publicProjectDTO[], totalPage: number, areas: number[] }> | commonOutput> {
      const { data, totalPage, areas } = await this._projectRepository.findStatusBaseProject(input)
