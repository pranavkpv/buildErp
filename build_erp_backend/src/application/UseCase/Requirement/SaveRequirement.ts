...
export class SaveRequirementUseCase implements ISaveRequirementUseCase {
   ...
   async execute(input: saveRequirementInput): Promise<commonOutput | commonOutput<userBaseProjectDTO>> {
      ...
      for (let element of spec_id) {
         ...
         for (let item of specDetails.labourDetails) {
         ...
         let materialSum = materials.reduce((sum, num) => sum += num.quantity * (num.unit_rate || 1), 0)
         let labourSum = labours.reduce((sum, num) => sum += (num.daily_wage || 1) * num.numberoflabour, 0)
         let additionalAmount = specDetails.additionalExpense_per * (labourSum + materialSum) / 100
         let profitAmount = specDetails.profit_per * (labourSum + materialSum + additionalAmount) / 100
         let specFinalAmount = materialSum + labourSum + additionalAmount + profitAmount
      ...
      for (let element of specData) {
         ...
         for (let item of element.labourDetails) {
            if (!item.daily_wage) {
