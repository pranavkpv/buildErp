...
export class ReceiveTransferUseCase implements IReceiveTransferUseCase {
    ...
    async execute(id: string, date: string): Promise<commonOutput<TransferOutput[]> | commonOutput>{
        const data  = await this._transferRepository.findTransferDataByToProjectAndDate(id, date);
        if (!data) {
