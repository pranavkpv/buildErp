...
export class MaterialRepository implements IMaterialRepository {
  ...
  async getMaterialById(_id: string): Promise<editMaterialFullDatafetch | null> {
  ...
  async getMaterialByBrandId(brand_id: string): Promise<IMaterialModelEntity | null> {
    return await materialDB.findOne({ brand_id });
  }

  // Get material by category
  async getMaterialByCategoryId(category_id: string): Promise<IMaterialModelEntity | null> {
    return await materialDB.findOne({ category_id });
  }

  //  Get material by unit
  async getMaterialByUnitId(unit_id: string): Promise<IMaterialModelEntity | null> {
  ...
  async getUnitsByMaterialName(material_name: string): Promise<string[]> {
  ...
  async getBrandsByMaterialName(material_name: string): Promise<string[]> {
