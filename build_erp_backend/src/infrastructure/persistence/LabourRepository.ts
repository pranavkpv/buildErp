 ...
 */
export class LabourRepository implements ILabourRepositoryEntity {

   /**
    * Fetches a paginated list of labour records with optional search filter.
    * @param input - Contains page number and search keyword.
    * @returns An object containing:
    *  - `data`: Array of labours
    ...
    */
   async saveLabour(input: inputLabour): Promise<void> {
   ...
   /**
    * Calculates the total wage for a list of labours.
    * @param labours - Array containing labour IDs and number of labours.
    * @returns The total sum of wages.
    */
   async findSumofLabouor(labours: { labour_id: string; numberoflabour: number; }[]): Promise<number> {
      let sum = 0;
      for (const element of labours) {
