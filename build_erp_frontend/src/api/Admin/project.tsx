...
export const deleteProjectData = async (data: deleteData) => {
   const response = await axioInstance.delete(`/admin/project`, { data });
   return response.data
}
...
export const putProject = async (_id: string, project_name: string, user_id: string, address: string,
   mobile_number: string, email: string, area: number, description: string) => {
   const response = await axioInstance.put(`/admin/project`, {
      _id,
      project_name,
      user_id,
      address,
