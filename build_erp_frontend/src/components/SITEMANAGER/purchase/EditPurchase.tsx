...
type listMaterial = {
   ...
   material_id: string;
   quantity: number;
   brands: string[]; // Add brands array to store brand options for this row
   units: string[]; // Add units array to store unit options for this row
};
...
function EditPurchase({ editId, editEnable, setEditEnable, onEditSuccess, editData }: editProps) {
   ...
   const editPurchaseFun = async (e: React.FormEvent) => {
      e.preventDefault();
      if (!validateForm()) {
   ...
   return (
      <div className="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-50 w-full">
         <div className="bg-gray-800 ml-20 rounded-lg p-6 max-w-6xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={editPurchaseFun} className="space-y-4">
