...
function EditPurchase({ editId, editEnable, setEditEnable, onEditSuccess, editData }) {
    if (!editEnable)
        return null;
    ...
    // Fetch brands and units for a specific row
    const giveBrandAndUnit = async (materialName, idx) => {
        ...
        newRow[idx] = {
            ...
            unit_rate: 0,
            material_id: "",
            brands: brandData.data || [], // Store brands for this row
            units: unitData.data || [], // Store units for this row
        };
        setRow(newRow);
    };
    ...
    // Preload brands and units for existing materials in editData
    useEffect(() => {
        fetchMaterial();
        fetchProject();
        // Fetch brands and units for each material in editData
