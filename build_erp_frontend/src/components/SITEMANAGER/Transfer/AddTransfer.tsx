...
function AddTransfer({ addEnable, setAddEnable, onAddSuccess }: setAdd) {
   ...
   const [Fromproject_id, setFromProjectId] = useState("");
   const [ToProject_id, setToProjectId] = useState("");
   ...
   const validateForm = () => {
      ...
      if (!Fromproject_id) newErrors.project = "From Project is required";

      if (!transfer_id) newErrors.invoice = "Invoice number is required";
   ...
   const addPurchaseFun = async (e: React.FormEvent) => {
      ...
      const response = await saveTransferApI(Fromproject_id,ToProject_id, transfer_id, date, description, materialDetails);
      if (response.success) {
   ...
   const ToProjectsetFun = async (_id:string)=>{
       const response = await ToProjectFetchAPI(_id)
       if(response.success){
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                     <select
                        ...
                        onChange={(e) => {
                           ...
                           ToProjectsetFun(e.target.value)
                        }}
                  ...
                  <div>
                     <input
                        ...
                        value={transfer_id}
                        onChange={(e) => setTransfer_id(e.target.value)}
                        className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-400 text-white"
                     />
