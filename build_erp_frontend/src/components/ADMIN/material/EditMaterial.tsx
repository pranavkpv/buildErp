...
function EditMaterial({ setEditEnable, editEnable, setEditId, editId, refreshData }: editMatDat) {
  ...
  const fetchData = async () => {
    ...
    const transformedProjectData = editData.data.projectStockData.map(
      (item: { project_id: string; stock: number; _id: string }) => ({
  ...
  const updateMaterial = async () => {
    ...
    if (materialName.trim() === "") {
      if (materialNameRef.current) materialNameRef.current.innerText = "Material name is required.";
      hasError = true;
    }
    if (selectUnitId === "") {
      if (unitRef.current) unitRef.current.innerText = "Unit is required.";
      hasError = true;
    }
    if (selectCategoryId === "") {
      if (categoryRef.current) categoryRef.current.innerText = "Category is required.";
      hasError = true;
    }
    if (selectBrandId === "") {
      if (brandRef.current) brandRef.current.innerText = "Brand is required.";
      hasError = true;
    }
    if (unit_rate <= 0) {
      if (unitRateRef.current) unitRateRef.current.innerText = "Unit rate must be greater than 0.";
      hasError = true;
    }
