...
function EditEstimation({ editEnable, setEditEnable, anEditSuccess, projectIds, editProjectId }) {
    if (!editEnable)
        return null;
    ...
    return (_jsx(_Fragment, { children: _jsx("div", { className: "fixed inset-0 bg-gray-900/80 flex items-center justify-center z-50 p-4 sm:p-6", children: _jsxs("div", { className: "relative bg-gray-800/90 backdrop-blur-sm rounded-xl shadow-2xl p-6 sm:p-8 w-full max-w-4xl max-h-[95vh] overflow-y-auto border border-gray-700/50", children: [_jsx("h1", { className: "text-2xl font-bold text-center text-gray-100 mb-6 border-b border-gray-700 pb-4", children: "Edit Estimation" }), _jsx("div", { className: "grid grid-cols-1 gap-6 mb-6", children: _jsxs("div", { children: [_jsx("label", { htmlFor: "projectSelect", className: "block text-sm font-medium text-gray-200 mb-1", children: "Project" }), _jsxs("select", { value: projectId, id: "projectSelect", "aria-label": "Select a project", className: "w-full px-4 py-2.5 bg-gray-700/50 border border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-colors duration-200 text-gray-100 placeholder-gray-400 text-sm", onChange: (e) => {
                                        ...
                                        setProjectId(e.target.value);
                                            ...
                                            setRow([...row, { spec_id: "", spec_name: "", unitrate: 0, quantity: 0, total: 0 }]);
                                                            ...
                                                            }, value: element.spec_id, children: [_jsx("option", { value: "", children: "Select Spec ID" }), spec.map((item) => (_jsx("option", { value: item.spec_id, children: item.spec_id }, item.spec_id)))] }) }), _jsx("td", { className: "px-6 py-4", children: _jsx("input", { type: "text", className: "w-full px-3 py-2 bg-gray-700/50 border border-gray-600 rounded-lg text-gray-100 placeholder-gray-400 text-sm", value: element.spec_name, readOnly: true, placeholder: "Spec Name" }) }), _jsx("td", { className: "px-6 py-4", children: _jsx("input", { type: "number", className: "w-full px-3 py-2 bg-gray-700/50 border border-gray-600 rounded-lg text-gray-100 placeholder-gray-400 text-sm", value: element.unitrate === 0 ? "" : element.unitrate, placeholder: "Unit Rate", readOnly: true }) }), _jsx("td", { className: "px-6 py-4", children: _jsx("input", { type: "text", disabled: loadOn, className: "w-full px-3 py-2 bg-gray-700/50 border border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-colors duration-200 text-gray-100 placeholder-gray-400 text-sm", placeholder: "Enter quantity", onChange: (e) => {
                                                                const updateRow = [...row];
                                                                updateRow[index].quantity = Number(e.target.value);
                                                                updateRow[index].total = updateRow[index].unitrate * updateRow[index].quantity;
                                                                setRow(updateRow);
                                                            }, value: element.quantity === 0 ? "" : element.quantity }) }), _jsx("td", { className: "px-6 py-4", children: _jsx("input", { type: "number", className: "w-full px-3 py-2 bg-gray-700/50 border border-gray-600 rounded-lg text-gray-100 placeholder-gray-400 text-sm", value: element.total === 0 ? "" : element.total, placeholder: "Total", readOnly: true }) }), _jsx("td", { className: "px-6 py-4 text-center", children: _jsx("button", { type: "button", disabled: loadOn, className: "text-red-400 hover:text-red-300 p-2 rounded-md hover:bg-gray-600/50 transition-all duration-200", "aria-label": "Delete specification row", onClick: () => {
                                                                const updateRow = row.filter((_, itemIndex) => itemIndex !== index);
                                                                setRow(updateRow);
                                                            }, children: _jsx(MinusCircleIcon, { className: "h-5 w-5" }) }) })] }, index)))) })] }) })] }), _jsxs("div", { className: "flex justify-between items-center mt-8 pt-4 border-t border-gray-700", children: [_jsxs("p", { className: "text-lg font-semibold text-gray-100", children: ["Final Amount: \u20B9", finalAmount.toLocaleString()] }), _jsxs("div", { className: "flex justify-end gap-4", children: [_jsx("button", { type: "button", disabled: loadOn, className: "bg-gray-600/90 hover:bg-gray-700 text-gray-100 px-5 py-2.5 rounded-lg shadow-sm hover:shadow-md transition-all duration-200 text-sm font-medium", onClick: () => setEditEnable(false), children: "Cancel" }), _jsx("button", { type: "button", disabled: loadOn, onClick: UpdateEstimation, className: "bg-teal-500/90 hover:bg-teal-600 text-white px-5 py-2.5 rounded-lg shadow-sm hover:shadow-md transition-all duration-200 text-sm font-medium", children: "Save Estimation" })] })] }), loadOn && (_jsx("div", { className: "absolute inset-0 bg-black/40 flex items-center justify-center rounded-xl pointer-events-none z-50", children: _jsx(Loading, {}) }))] }) }) }));
}
